...
def extract_realsense_imu_to_json(bag_path: pathlib.Path, json_path: pathlib.Path, imu_topics: list = DEFAULT_IMU_TOPICS):
    """
    BAG 파일에서 Accel/Gyro 토픽을 읽어 JSON 형식으로 통합 저장합니다.
    사용자 요청에 따라 평면(flattened) 구조를 사용하며, 기존 파일이 존재하면 덮어씁니다.
    """
    all_samples = []
    ...
    try:
        ...
        with rosbag.Bag(str(bag_path), 'r') as bag:
            for topic, msg, t in bag.read_messages(topics=imu_topics):
                ...
                sample = {
                    'value': [], 'cts': cts_ms, 'date': date_str,
...
def extract_realsense_frames(bag_path: pathlib.Path, output_dir: pathlib.Path, 
                             color_topic: str = DEFAULT_COLOR_TOPIC, 
                             depth_topic: str = DEFAULT_DEPTH_TOPIC):
    """
    Color와 Depth 프레임을 PNG 파일로 저장하고, TUM 포맷 타임스탬프 목록을 생성합니다.
    기존의 color/ 및 depth/ 폴더는 덮어쓰기 위해 삭제 후 다시 생성됩니다.
    """
    if bridge is None:
        return -1
    ...
    try:
        with rosbag.Bag(str(bag_path), 'r') as bag:
            for topic, msg, t in bag.read_messages(topics=topics_to_read):
                ...
                if topic == color_topic:
                    # BGR8로 변환하여 PNG 저장 (OpenCV 기본 컬러 순서)
                    ...
                    # 16UC1 (16비트 부호 없는 정수)로 변환
                    cv_image = bridge.imgmsg_to_cv2(msg, desired_encoding="passthrough")
                    
                    # 16비트 PNG 저장을 위해 cv2.IMWRITE_PNG_COMPRESSION 옵션 사용
                    filename = f"{timestamp_str}.png"
                    # 압축 수준 0(압축 없음)
                    cv2.imwrite(str(depth_dir.joinpath(filename)), cv_image, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                    depth_timestamps.append(f"{timestamp_str} depth/{filename}")

        # 타임스탬프 목록을 TUM 포맷으로 저장 (덮어쓰기)
        with open(output_dir.joinpath('rgb.txt'), 'w') as f:
            f.write("# timestamp filename\n")
            f.write('\n'.join(rgb_timestamps))
            
        with open(output_dir.joinpath('depth.txt'), 'w') as f:
            f.write("# timestamp filename\n")
        ...
        print(f"Error processing {bag_path.name} (Frames): {e}")
        return -1
...
@click.option('-c', '--color_topic', default=DEFAULT_COLOR_TOPIC, help="RealSense Color topic.")
@click.option('-d', '--depth_topic', default=DEFAULT_DEPTH_TOPIC, help="RealSense Depth topic.")
...
def main(imu_topic, color_topic, depth_topic, num_workers, session_dir):
    ...
    print("--- Starting RealSense BAG Data Extraction for SLAM ---")
    ...
    for session in session_dir:
        ...
        if not input_bag_paths:
            print(f"Warning: No 'raw_bag.bag' found in directories under {session_path}. Skipping.")
            continue

        print(f'Found {len(input_bag_paths)} BAG files for full extraction.')

        total_tasks = len(input_bag_paths) * 2

        with tqdm(total=total_tasks) as pbar: 
            with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
                futures = set()
                
                for bag_path in tqdm(input_bag_paths, desc="Scheduling full extraction"):
                    bag_dir = bag_path.parent 
                    imu_json_path = bag_dir.joinpath('imu_data.json')

                    # 1. IMU 추출 작업 예약 (JSON)
                    imu_future = executor.submit(
                        extract_realsense_imu_to_json, bag_path, imu_json_path, imu_topics_to_check)
                    futures.add(imu_future)
                    
                    # 2. RGB/Depth 프레임 및 타임스탬프 추출 작업 예약 (PNG + TXT)
                    frame_future = executor.submit(
                        extract_realsense_frames, bag_path, bag_dir, color_topic, depth_topic)
        ...
        # 결과 요약
        results = [x.result() for x in completed if x.result() != 0 and x.result() != -1]
        errors = [x.result() for x in completed if x.result() == -1]
        
        print("\nDone! Summary:")
        print(f"  Total successful extractions: {len(results)}")
