...
def main(input, output, robot_config, 
    ...
    with SharedMemoryManager() as shm_manager:
        with Spacemouse(shm_manager=shm_manager) as sm, \
            ...
            # ===== End prediction-only logging setup =====
            ...
            while True:
                ...
                try:
                    ...
                    for _i in range(1):
                        ...
                        full_horizon_prediction = action.copy()
                        ...
                        # ===== Prediction-only logging =====
                        # Vector layout:
                        # - current_pose has shape (num_robots, 7).
                        # - full_horizon_prediction has shape (action_horizon, 7 * num_robots).
                        # - For each robot, the 7 values are:
                        #   [x, y, z, rx, ry, rz, gripper_width]
                        #   x/y/z: end-effector position in meters.
                        #   rx/ry/rz: end-effector rotation as axis-angle / rotation-vector.
                        #   gripper_width: gripper opening width in meters.
                        # - If there are multiple robots, robot0 uses columns 0:7,
                        #   robot1 uses columns 7:14, and so on.
                        current_pose = list()
                        for robot_idx in range(len(robots_config)):
                            current_pose.append(np.concatenate([
                        ...
                        # No robot command is sent in this script, so the executed count is 0.
                        # The would-execute count is how many actions eval_real.py would have sent.
                        num_actions_executed = 0
                        num_actions_that_would_execute = len(this_target_poses)
                        pred_log_record = {
                            "phase": "policy",
                            ...
                            "num_actions_that_would_execute": int(num_actions_that_would_execute),
                        }
                        ...
                        print("current_pose:")
                        print(np.round(current_pose, 4))
                        print("full_horizon_prediction:")
                        print(np.round(full_horizon_prediction, 4))
                        print("num_actions_executed:", num_actions_executed)
                        print("num_actions_that_would_execute:", num_actions_that_would_execute)
                        pred_log_idx += 1
