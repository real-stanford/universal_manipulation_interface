...
def create_masked_image(video_path, mask_path, output_path):
    ...
    if frame.shape[:2] != mask.shape[:2]:
        print(f"Warning: Frame resolution ({frame.shape[:2]}) and mask resolution ({mask.shape[:2]}) are different. Resizing mask to fit frame size.")
...
if __name__ == "__main__":
    ...
    video_sample_path = "~/Documents/test/1920_gripper_30hz.mp4"
